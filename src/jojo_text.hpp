...
#include "stb_font_consolas_32_latin_ext.inl"
...
/**
 * lifted almost 1:1 from saschawillems
 */
class TextOverlay {
private:
    ...
    stb_fontchar stbFontData[STB_FONT_consolas_32_latin_ext_NUM_CHARS];
    uint32_t numLetters;
public:
            ...
            std::vector<VkPipelineShaderStageCreateInfo> shaderstages)
    {
        ...
        this->chosenDevice = physicalDevice;
        this->queue = queue;
    ...
    {
        const uint32_t fontWidth = STB_FONT_consolas_32_latin_ext_BITMAP_WIDTH;
        const uint32_t fontHeight = STB_FONT_consolas_32_latin_ext_BITMAP_WIDTH;

        static unsigned char font24pixels[fontWidth][fontHeight];
        stb_font_consolas_32_latin_ext(stbFontData, font24pixels, fontHeight);
        ...
        VK_CHECK_RESULT(vkCreateCommandPool(logicalDevice, &cmdPoolInfo, nullptr, &commandPool));
        ...
        VK_CHECK_RESULT(vkAllocateCommandBuffers(logicalDevice, &cmdBufAllocateInfo, cmdBuffers.data()));
        ...
        allocInfo.memoryTypeIndex = findMemoryTypeIndex(chosenDevice, memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        ...
        allocInfo.allocationSize = memReqs.size;
        allocInfo.memoryTypeIndex = findMemoryTypeIndex(chosenDevice, memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        ...
        VK_CHECK_RESULT(vkAllocateMemory(logicalDevice, &allocInfo, nullptr, &stagingBuffer.memory));
        VK_CHECK_RESULT(vkBindBufferMemory(logicalDevice, stagingBuffer.buffer, stagingBuffer.memory, 0));

        uint8_t *data;
        VK_CHECK_RESULT(vkMapMemory(logicalDevice, stagingBuffer.memory, 0, allocInfo.allocationSize, 0, (void **)&data));
        ...
        VK_CHECK_RESULT(vkCreateImageView(logicalDevice, &imageViewInfo, nullptr, &view));
        ...
        VK_CHECK_RESULT(vkCreateDescriptorSetLayout(logicalDevice, &descriptorSetLayoutInfo, nullptr, &descriptorSetLayout));
        ...
        VkPipelineLayoutCreateInfo pipelineLayoutInfo =
                vks::initializers::pipelineLayoutCreateInfo(
                        &descriptorSetLayout,
                        1);
        VK_CHECK_RESULT(vkCreatePipelineLayout(logicalDevice, &pipelineLayoutInfo, nullptr, &pipelineLayout));
    ...
    void preparePipeline()
    {
        ...
        VK_CHECK_RESULT(vkCreateGraphicsPipelines(logicalDevice, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline));
    }
    ...
    {
        ASSERT_VULKAN(vkMapMemory(logicalDevice, memory, 0, VK_WHOLE_SIZE, 0, (void **)&mapped));
        numLetters = 0;
    }
    ...
    {
        const uint32_t firstChar = STB_FONT_consolas_32_latin_ext_FIRST_CHAR;
    ...
    void updateCommandBuffers()
    {
        ...
        for (int32_t i = 0; i < cmdBuffers.size(); ++i)
        {
            ...
            ASSERT_VULKAN(vkBeginCommandBuffer(cmdBuffers[i], &cmdBufInfo));
            ...
            ASSERT_VULKAN(vkEndCommandBuffer(cmdBuffers[i]));
        }
    }
